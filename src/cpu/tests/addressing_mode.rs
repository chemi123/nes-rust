...
fn test_indirect_x_zero_page_wrap() {
    let mut cpu = Cpu::new(NESBus::with_program(&[LDX_IMMEDIATE, 0x1F, LDA_IMMEDIATE, 0x00, ADC_INDIRECT_X, 0xF0, BRK]));
...
fn test_indirect_x_pointer_at_0xff() {
    let mut cpu = Cpu::new(NESBus::with_program(&[LDX_IMMEDIATE, 0x00, LDA_IMMEDIATE, 0x00, ADC_INDIRECT_X, 0xFF, BRK]));
...
fn test_indirect_y_pointer_at_0xff() {
    let mut cpu = Cpu::new(NESBus::with_program(&[LDY_IMMEDIATE, 0x05, LDA_IMMEDIATE, 0x00, ADC_INDIRECT_Y, 0xFF, BRK]));
    // base=0xFF
    // low=mem[0xFF]=0x80, high=mem[0x00]=0x02 (wraps to zero page!)
