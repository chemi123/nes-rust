...
fn test_adc_immediate() {
    let mut cpu = Cpu::new(NESBus::with_program(&[LDA_IMMEDIATE, 0x10, ADC_IMMEDIATE, 0x20, BRK]));
...
fn test_adc_carry_flag() {
    let mut cpu = Cpu::new(NESBus::with_program(&[LDA_IMMEDIATE, 0xFF, ADC_IMMEDIATE, 0x01, BRK]));
...
fn test_adc_overflow_positive() {
    let mut cpu = Cpu::new(NESBus::with_program(&[LDA_IMMEDIATE, 0x50, ADC_IMMEDIATE, 0x50, BRK]));
...
fn test_adc_overflow_negative() {
    let mut cpu = Cpu::new(NESBus::with_program(&[LDA_IMMEDIATE, 0x80, ADC_IMMEDIATE, 0x80, BRK]));
...
fn test_adc_no_overflow() {
    let mut cpu = Cpu::new(NESBus::with_program(&[LDA_IMMEDIATE, 0x50, ADC_IMMEDIATE, 0x10, BRK]));
...
fn test_adc_zero_page() {
    let mut cpu = Cpu::new(NESBus::with_program(&[LDA_IMMEDIATE, 0x03, ADC_ZERO_PAGE, 0x10, BRK]));
...
fn test_adc_absolute() {
    let mut cpu = Cpu::new(NESBus::with_program(&[LDA_IMMEDIATE, 0x03, ADC_ABSOLUTE, 0x00, 0x02, BRK]));
...
fn test_sbc_immediate() {
    let mut cpu = Cpu::new(NESBus::with_program(&[SEC_IMPLIED, LDA_IMMEDIATE, 0x50, SBC_IMMEDIATE, 0x10, BRK]));
...
fn test_sbc_carry_clear() {
    let mut cpu = Cpu::new(NESBus::with_program(&[LDA_IMMEDIATE, 0x50, SBC_IMMEDIATE, 0x10, BRK]));
...
fn test_sbc_underflow() {
    let mut cpu = Cpu::new(NESBus::with_program(&[LDA_IMMEDIATE, 0x00, SBC_IMMEDIATE, 0x01, BRK]));
...
fn test_sbc_overflow() {
    let mut cpu = Cpu::new(NESBus::with_program(&[SEC_IMPLIED, LDA_IMMEDIATE, 0x50, SBC_IMMEDIATE, 0xB0, BRK]));
...
fn test_sbc_zero_page() {
    let mut cpu = Cpu::new(NESBus::with_program(&[SEC_IMPLIED, LDA_IMMEDIATE, 0x08, SBC_ZERO_PAGE, 0x10, BRK]));
    cpu.bus.write(0x10, 0x05);
    // 0x08 - 0x05 = 0x03
    cpu.run_until_break().unwrap();
