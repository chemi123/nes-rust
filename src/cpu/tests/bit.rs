...
fn test_bit_zero_flag_set() {
    ...
    cpu.run(&[LDA_IMMEDIATE, 0xF0, BIT_ZERO_PAGE, 0x10, BRK]);
...
fn test_bit_zero_flag_clear() {
    ...
    cpu.run(&[LDA_IMMEDIATE, 0x01, BIT_ZERO_PAGE, 0x10, BRK]);
...
fn test_bit_negative_from_memory() {
    let mut cpu = Cpu::new();
    cpu.memory.write(0x10, 0x80); // bit7=1
    // A=0x00, BIT $10 -> Negative = value bit7 = 1
    cpu.run(&[LDA_IMMEDIATE, 0x00, BIT_ZERO_PAGE, 0x10, BRK]);
...
fn test_bit_overflow_from_memory() {
    let mut cpu = Cpu::new();
    cpu.memory.write(0x10, 0x40); // bit6=1
...
fn test_bit_does_not_modify_accumulator() {
    let mut cpu = Cpu::new();
    cpu.memory.write(0x10, 0xFF);
    cpu.run(&[LDA_IMMEDIATE, 0x42, BIT_ZERO_PAGE, 0x10, BRK]);
...
fn test_bit_all_flags() {
    let mut cpu = Cpu::new();
    cpu.memory.write(0x10, 0xC0); // bit7=1, bit6=1
    // A=0xC0, BIT $10 -> A & 0xC0 = 0xC0 (not zero), Negative=1, Overflow=1
    cpu.run(&[LDA_IMMEDIATE, 0xC0, BIT_ZERO_PAGE, 0x10, BRK]);
    assert_eq!(cpu.processor_status & 0b0000_0010, 0); // no Zero
    assert_eq!(cpu.processor_status & 0b1000_0000, 0b1000_0000); // Negative
    assert_eq!(cpu.processor_status & 0b0100_0000, 0b0100_0000); // Overflow
}
...
fn test_bit_absolute() {
    let mut cpu = Cpu::new();
    cpu.memory.write(0x0200, 0xC0);
    cpu.run(&[LDA_IMMEDIATE, 0x00, BIT_ABSOLUTE, 0x00, 0x02, BRK]);
    assert_eq!(cpu.processor_status & 0b0000_0010, 0b10); // Zero
    assert_eq!(cpu.processor_status & 0b1000_0000, 0b1000_0000); // Negative
    assert_eq!(cpu.processor_status & 0b0100_0000, 0b0100_0000); // Overflow
}
