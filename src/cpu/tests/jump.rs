...
fn test_jmp_absolute() {
    let mut cpu = Cpu::new(NESBus::with_program(&[JMP_ABSOLUTE, 0x05, 0x80, INX_IMPLIED, INX_IMPLIED, INY_IMPLIED, BRK]));
...
fn test_jmp_indirect() {
    let mut cpu = Cpu::new(NESBus::with_program(&[JMP_INDIRECT, 0x00, 0x02, INX_IMPLIED, INX_IMPLIED, INX_IMPLIED, INX_IMPLIED, INY_IMPLIED, BRK]));
...
fn test_jmp_indirect_page_boundary_bug() {
    let mut cpu = Cpu::new(NESBus::with_program(&[JMP_INDIRECT, 0xFF, 0x02, INX_IMPLIED, INX_IMPLIED, INX_IMPLIED, INX_IMPLIED, INY_IMPLIED, BRK]));
...
#[test]
fn test_jsr_rts() {
    let mut cpu = Cpu::new(NESBus::with_program(&[JSR_ABSOLUTE, 0x05, 0x80, INX_IMPLIED, BRK, INY_IMPLIED, RTS_IMPLIED]));
...
fn test_jsr_pushes_return_address() {
    let mut cpu = Cpu::new(NESBus::with_program(&[JSR_ABSOLUTE, 0x05, 0x80, BRK, BRK, BRK]));
