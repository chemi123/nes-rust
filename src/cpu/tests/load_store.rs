...
fn test_lda_indirect_x() {
    let mut cpu = Cpu::new(NESBus::with_program(&[LDX_IMMEDIATE, 0x10, LDA_INDIRECT_X, 0x05, BRK]));
...
fn test_lda_indirect_y() {
    let mut cpu = Cpu::new(NESBus::with_program(&[LDY_IMMEDIATE, 0x05, LDA_INDIRECT_Y, 0x20, BRK]));
...
fn test_sta_indirect_x() {
    let mut cpu = Cpu::new(NESBus::with_program(&[LDX_IMMEDIATE, 0x10, LDA_IMMEDIATE, 0x55, STA_INDIRECT_X, 0x05, BRK]));
    // pointer at (0x05 + 0x10) = 0x15 -> 0x0300
    cpu.bus.write(0x15, 0x00);
    cpu.bus.write(0x16, 0x03);
    cpu.run().unwrap();
...
fn test_sta_indirect_y() {
    let mut cpu = Cpu::new(NESBus::with_program(&[LDY_IMMEDIATE, 0x05, LDA_IMMEDIATE, 0x55, STA_INDIRECT_Y, 0x20, BRK]));
    // pointer at 0x20 -> 0x0300, + Y(0x05) = 0x0305
    cpu.bus.write(0x20, 0x00);
    cpu.bus.write(0x21, 0x03);
    cpu.run().unwrap();
